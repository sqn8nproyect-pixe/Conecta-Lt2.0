...
// Legacy seed data — used only by prisma/seed.ts to populate PostgreSQL.
// Runtime data comes from the API (src/lib/api.ts → /api/businesses).
// `slug` is omitted because the seed script generates it with slugify(name).
type SeedEstablishment = Omit<Establishment, 'slug'>;

// 21 Establishments with enhanced properties
export const establishments: SeedEstablishment[] = [
  {
